...
from .device import Device, Props
from .deviceinfo import DeviceInfo
from .mqtt_client import GreeMqttClient, MqttDeviceMessage
...
class CloudDevice(Device):
    ...
    async def update_state(self):
        ...
        try:
            ...
            _LOGGER.warning(f"Timeout waiting for state update from {self.device_info.name} (timeout={self._command_timeout}s)")
        finally:
    ...
    async def _send_command(self, opt: List[str], p: List[Any]) -> None:
        ...
        try:
            ...
            _LOGGER.warning(f"Timeout waiting for command response from {self.device_info.name}")        except Exception as e:
            _LOGGER.error(f"Error updating state for {self.device_info.name}: {e}")        finally:
            self._response_event = None
    
    def _handle_mqtt_message(self, topic: str, message: MqttDeviceMessage) -> None:
        """Handle incoming MQTT messages"""
        _LOGGER.debug(f"Handling MQTT message for {self.device_info.name}: topic={topic}")
        
        # Check if message is for this device
        if self._parent_mac not in topic and self._child_mac not in topic:
            return
        
        _LOGGER.debug(f"Message matched for device {self.device_info.name}: t={message.t}")
